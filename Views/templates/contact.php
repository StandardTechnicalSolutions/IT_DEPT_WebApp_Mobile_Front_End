<?php 
    /**
     * The placeholder content that will fill the main web page template
     * when the user wants to visit the home page
     */
    $placeholderHeader = "<div class='temp-header'></div>";
    $placeholder1 = "<li><a href='/?location=home'>Home</a></li>";
    $placeholder2 = "<li><a href='/?location=equipment'>Equipment</a></li>";
    $placeholder3 = "<li><a href='/?location=map'>Map</a></li>";
    $placeholder4 = "<li><a href='/?location=contact'>Contact</a></li>";
    $placeholder5 = "<li><a href='/?location=orderForm'>Order Scantron</a></li>";
    $placeholder6 = "<li><a href='/?location=reporttype'>Report Type</a></li>";

    $content_placeholder = "
        <h3 class='contact-styles'>Bloomsburg University Exam Scanning Services</h1>
        <h4 class='contact-styles'>Ben Franklin Hall - Room 120</h3>
        <h5 class='contact-styles'>Spring and Fall Hours</h4>
        <p class='contact-styles'>Monday - Thursday 08:00a - 07:00p</p>
        <p class='contact-styles'>Friday 08:00a - 4:30p</p>
        <h5 class='contact-styles'>Summer Hours</h4>
        <p class='contact-styles'>Monday - Friday 08:00a - 4:00p</p>
        <h4 class='contact-styles'>Phone Number (570)-389-4099</h3>
        <h5 class='contact-styles'>Email scanner@bloomu.edu</h4>
    ";
    $extraJsFunction = "";
